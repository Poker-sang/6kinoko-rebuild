...
void class_contract(HSQUIRRELVM vm) {
    ...
    require((int32_t)(intptr_t)(kinoko_sqplus_construct_class_binding((void *)(state.data() + 1), "UnalignedBinding", (const char *)(intptr_t)(0))) == address(state.data() + 1), "unaligned class binding result");
...
void lookup_contract(HSQUIRRELVM vm) {
    ...
    for (const char* key : {"short", "wrong"}) {
        ...
        context[0] = 2;
        require(kinoko_sqplus_find_table_variable(&found, context) == -1, "safe table malformed metadata");
        require(kinoko_sqplus_find_variable(context, &found) == -1, "safe raw malformed metadata");
        sq_settop(vm, 0);
    }
    require(kinoko_sqplus_find_table_variable(&found, nullptr) == -1, "null metadata context");
}
...
void values_contract(HSQUIRRELVM vm) {
    ...
    for (int size : {1, 2, 4}) {
        ...
        for (int32_t input : {-32769, -129, -1, 0, 127, 128, 32767, 65535, 65536}) {
            ...
            require(kinoko_sqplus_write_variable(context, (const void*)(uintptr_t)(info_addr), (void*)(uintptr_t)(value_addr)) == 1, "integer setter returns one");
    ...
    for (int size : {1, 2, 4}) {
        ...
        for (int32_t input : {-1, 0, 128, 65535, 0x12345678}) {
            ...
            require(kinoko_sqplus_write_variable(context, (const void*)(uintptr_t)(info_addr), (void*)(uintptr_t)(value_addr)) == 1 && integer(vm) == input,
    ...
    require(kinoko_sqplus_write_variable(context, (const void*)(uintptr_t)(info_addr), (void*)(uintptr_t)(value_addr)) == 1 && load<int32_t>(value_addr) == 0, "legacy invalid integer conversion writes zero");
    info.category = 2; store(info_addr, info);
    store(value_addr, 42.5f); sq_pop(vm, 1);
    require(kinoko_sqplus_write_variable(context, (const void*)(uintptr_t)(info_addr), (void*)(uintptr_t)(value_addr)) == -1 && load<float>(value_addr) == 42.5f, "invalid float conversion leaves destination unchanged");
    sq_settop(vm, 2); sq_pushinteger(vm, 17);
    require(kinoko_sqplus_write_variable(context, (const void*)(uintptr_t)(info_addr), (void*)(uintptr_t)(value_addr)) == 1 && numeric(vm) == 17, "float property accepts integer conversion");
    ...
    for (int flag : {ReadOnly, Constant}) {
        ...
        require(kinoko_sqplus_write_variable(context, (const void*)(uintptr_t)(info_addr), (void*)(uintptr_t)(value_addr)) == -1, "immutable metadata blocks stores");
    }
    info = {0, 3, 0, 0, 1, 0}; store(info_addr, info);
    sq_settop(vm, 2); sq_pushinteger(vm, 1);
    require(kinoko_sqplus_write_variable(context, (const void*)(uintptr_t)(info_addr), (void*)(uintptr_t)(value_addr)) == 1 && load<uint8_t>(value_addr) == 0, "boolean setter rejects truthy integer conversion");
    sq_settop(vm, 2); sq_pushbool(vm, SQTrue);
    require(kinoko_sqplus_write_variable(context, (const void*)(uintptr_t)(info_addr), (void*)(uintptr_t)(value_addr)) == 1 && load<uint8_t>(value_addr) == 1, "boolean true round trip");
...
if(propertyActor.value != 123 || propertyActor.x != 7.25 || propertyActor.enabled != false) throw "property roundtrip";
    ...
    require(kinoko_sqplus_instance_get((struct SQVM *)(vm)) == 1 && integer(vm) == 123, "direct instance getter");
    sq_settop(vm, 0);
...
try { methodActor.xy(1,2.0);
    ...
    require(kinoko_sqplus_call_move((void *)(intptr_t)(add_address(address(&native), -8)), (void *)(reinterpret_cast<void*>(&two_float)), 8, (struct SQVM *)(vm), 1) == 0, "receiver offset adapter");
    require(native.calls == before + 1 && static_cast<uint32_t>(native.arguments[0]) == 0x80000000u, "float sign bit preserved");
    sq_settop(vm, 0);
    require(kinoko_sqplus_call_integer((void *)(&native), (void *)(reinterpret_cast<void*>(&one_int)), 0, (struct SQVM *)(vm), INT32_MAX) == -1, "large integer index checked");
    require(kinoko_sqplus_call_rectangle((void *)(&native), (void *)(reinterpret_cast<void*>(&four_float)), 0, (struct SQVM *)(vm), INT32_MAX) == -1, "float range overflow checked");
    require(kinoko_sqplus_argument_integer((struct SQVM *)(vm), 1) == 0 && kinoko_sqplus_argument_float((struct SQVM *)(vm), INT32_MIN) == 0, "invalid conversion outputs initialized");
...
void mapped_property_contract(HSQUIRRELVM vm) {
    ...
    require(kinoko_sqplus_instance_set((struct SQVM *)(vm)) == 1 && integer(vm) == -123 && mapped[1] == -123,
        "mapped property setter");
    require(native[0] == 12 && native[1] == 34 && mapped[0] == 56, "primary pointer and guards untouched");
    sq_settop(vm, 0);
    instance.view().push(vm); sq_setinstanceup(vm, -1, nullptr); sq_pushstring(vm, "value", -1);
    require(kinoko_sqplus_instance_get((struct SQVM *)(vm)) == 1 && integer(vm) == -123,
    ...
    instance.view().push(vm); sq_pushstring(vm, "value", -1);
    require(kinoko_sqplus_instance_get((struct SQVM *)(vm)) == 1 && integer(vm) == 29,
