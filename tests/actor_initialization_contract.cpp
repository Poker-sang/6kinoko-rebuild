...
KinokoActor *self() { return reinterpret_cast<KinokoActor *>(&actor); }
...
void clear() {
    for(auto member:script_members) kinoko_sqplus_object_destroy((actor.*member).data());
    if(actor.owner) { std::free(actor.owner); actor.owner=nullptr; }
    actor={}; refs.fill(0); copies.clear(); releases.clear(); strong=0;
}
}
...
HSQOBJECT sqplus_create_instance(HSQUIRRELVM,HSQOBJECT type) {
    ...
    if(replace_inputs) { // factory/release hooks may replace the caller's slots
        kinoko_sqplus_object_destroy(actor.initial_function.data());
        kinoko_sqplus_object_destroy(actor.initial_argument.data());
    }
...
int32_t kinoko_script_callback_invoke_owned(KinokoScriptCallback *call,KinokoOwnedObjectWords *arg,int32_t,int32_t) {
    ...
    CHECK(strong==1 && actor.owner && *actor.owner==self());
    ...
    actor.x=30; actor.y=40; actor.local_bounds={-2,-3,5,7};
...
int32_t kinoko_actor_step_callback(KinokoActor *) { actor.take=99; return 0; }
...
int main() {
    ...
    for(int mode=0;mode<4;++mode) {
        ...
        write(actor.initial_function.data(),fn); write(actor.initial_argument.data(),arg);
        actor.pool_handle=0x20042;
        ...
        try {
            CHECK(kinoko_actor_initialize(self(),nullptr,
                reinterpret_cast<const KinokoOwnedObjectWords *>(actor.initial_function.data()),1,2,-1,
                reinterpret_cast<const KinokoOwnedObjectWords *>(actor.initial_argument.data()))==1);
        } catch(const std::runtime_error&) { threw=true; }
        CHECK(threw==(mode!=0)); CHECK(copies[0]==2 && copies[1]==1);
        CHECK(releases[releases.size()-2]==1 && releases.back()==2);
        CHECK(actor.id==0x42);
        if(mode==1) { CHECK(!actor.owner && refs[1]==1 && refs[2]==1); }
        else if(mode==3) { CHECK(refs[1]==1 && refs[2]==1 && refs[3]==1); }
        else {
            CHECK(refs[1]==0 && refs[2]==0 && refs[3]==1);
            if(mode==0) CHECK(actor.world_bounds.left==28 && actor.world_bounds.top==37 && actor.world_bounds.right==35 && actor.previous_y==40);
        }
    }
    clear(); actor.take=7; write(actor.update_function.data(),{1,OT_CLOSURE,0});
    kinoko_actor_tick(self()); CHECK(actor.take==99 && advanced_take==7);
    clear();
}
