...
uint32_t kinoko_safe_c_string_length(const char* text) { return text ? static_cast<uint32_t>(std::strlen(text)) : 0; }
...
int main() {
    QueueRecord queue{};
    CHECK(kinoko_audio_manager_list_init(queue));
    CHECK(queue.head->empty());
    std::uint32_t value = 77;
    CHECK(!kinoko_audio_list_pop(queue.head, &value) && value == 77);
    for (std::uint32_t n = 0; n < 128; ++n) kinoko_audio_list_push(queue.head, n);
    for (std::uint32_t n = 0; n < 128; ++n) {
        CHECK(kinoko_audio_list_pop(queue.head, &value));
        CHECK(value == n);
    }
    CHECK(queue.head->empty());
    kinoko_audio_clear_queue(queue.head);
    ...
    CHECK(kinoko_audio_handle_create(&manager.handles, &first));
    CHECK(kinoko_audio_handle_create(&manager.handles, &second));
    CHECK(first == 0x10000 && second == 0x20001);
    auto* record = kinoko_audio_handle_lookup(&manager.handles, first);
    CHECK(record && record->gain == 1 && record->playback_state == 3 && !record->ready);
    CHECK(record->path.value->empty());
    CHECK(!kinoko_audio_handle_lookup(&manager.handles, first + 0x10000));
    CHECK(!kinoko_audio_handle_lookup(&manager.handles, 0xffff));
    const char path[] = "long-path-owned-by-buffer.cv3";
    record->path.value->assign(path, sizeof(path) - 1);
    CHECK(std::strcmp(kinoko_audio_buffer_path(record), path) == 0);
    ...
    Buffer source;
    {
        ...
        CHECK(kinoko_bgm_write_buffer(&track, samples, sizeof(samples)));
        ...
        kinoko_bgm_release_state(&moved);
    ...
    Buffer fade_buffer;
    {
        ...
        kinoko_bgm_begin_fade_locked(&track, 100, 0.0f, 0, true);
        track.fade_started = 10;
        update_track_fade(track, 110);
        CHECK(track.retirement_requested && fade_buffer.releases == 0);
        kinoko_bgm_release_state(&track);
    }
    ...
    CHECK(kinoko_audio_handle_lookup(&manager.handles, first) && retired_buffer.releases == 0);
    release_retired_requests_locked();
    CHECK(!kinoko_audio_handle_lookup(&manager.handles, first));
