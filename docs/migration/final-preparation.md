...
> 当前完成/剩余/待运行状态以 [2026-09-26 全项目核对](../project-status-20260926/README.md) 为准。本文历史阶段的待办和估计不自动代表当前缺口。
...
后续原版逻辑核对已移除图层构造时的348字节整块清零；构造函数现在只写入
41E390确实初始化的字段与属性区。未知字段和padding不由构造函数代写。
...
后续核对也已移除重建版额外的256 MiB索引上限；有界cursor仅在索引内容截断时阻止越界读取。
脚本读取恢复402D40的固定缓冲区末四字节替换，并移除了重建版额外的64 MiB与空文件拒绝。
少于四字节的路径在原版会越界写入，现只在该无定义输入上拒绝，不能称为原版规则。
