...
Deleted the replaced decompiled constructor, size and destructor bodies and the hand-linked pool Get/release implementations. The remaining flat-vector push helper serves input storage only. The outer ActorManager ownership list, priority tree, resource list, ACT grid/3D and rich-text paths remain unfinished; this is not a claim of complete library replacement. Added compiled-only lifecycle coverage for packed handles, stale lookup, LIFO reuse, allocated count and virtual destruction. No game, ctest or contract executable was run.
