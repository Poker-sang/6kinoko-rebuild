...
std::list<SQSharedState*>& owned_states() {
    static std::list<SQSharedState*> values;
    return values;
}
...
extern "C" void kinoko_sq_release_owned_states(void) {
    ...
    std::list<SQSharedState*> pending;
...
extern "C" int32_t kinoko_sqplus_release_vm_wrappers(void) {
    if (*host_slots().cached_root != 0) {
        (int32_t)(intptr_t)(kinoko_sqplus_object_destroy(*host_slots().cached_root));
        std::free(*host_slots().cached_root);
...
extern "C" void * kinoko_sqplus_root_object(void) {
    if (*host_slots().cached_root != 0) return *host_slots().cached_root;
    auto* vm = current_vm();
    if (!vm) return 0;
    sq_pushroottable(vm);
    ...
    *host_slots().current_vm = nullptr;
    if (!requested_vm) {
        ...
        try {
            ...
            (void)registered;
            *host_slots().newest_shared_state = &values.front();
        } catch(...) {
        ...
        ((int32_t)(uintptr_t)kinoko_sq_pop(current, 1));
    }
