...
public:
    NativeField(SQVM* id, bool trace, bool indirect = false, bool setter = false)
        : vm_(id), target_(0), offset_(0), storage_(nullptr) {
...
template<class T> int32_t set_number(SQVM* id, bool trace, bool indirect = false) {
...
extern "C" SQInteger kinoko_sqrat_set_string(HSQUIRRELVM vm) {
    NativeField field(vm, false, false, true);
    if (!field.storage()) return 0;
